...
```bash
docker run -it -v $(pwd)/db_backup:/tmp/psql_backup --rm ghcr.io/cristicriss17g/pg_actions:latest {host} {user} {source_db} {target_db} {user_owner}
```

if you do not wish to save the backup, you can omit the volume mount.

```bash
docker run -it --rm ghcr.io/cristicriss17g/pg_actions:latest {host} {user} {source_db} {target_db} {user_owner}
```
